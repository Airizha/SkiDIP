...
if page == " Overview":
    ...
    st.subheader(" AI Impact Level Distribution")
    if "AI IMPACT LEVEL" in nd.columns:
        ai_counts = nd["AI IMPACT LEVEL"].value_counts()
        fig, ax = new_fig(6, 4)
        wedge_colors = ["#4C4FF5", "#CF8000", "#438100", "#D0021B"]
        ax.pie(
            ai_counts.values, labels=ai_counts.index,
            autopct="%1.1f%%", colors=wedge_colors[:len(ai_counts)],
            startangle=140, pctdistance=0.8,
            wedgeprops={"edgecolor": "#0e1117", "linewidth": 1.5},
        )
        ax.set_title(f"AI Impact — {selected_industry}", color="#e0e0e0")
        plt.tight_layout()
        st.pyplot(fig)
        plt.close()
